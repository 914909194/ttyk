...
// 执行注册
	public function reg(){
		...
		if ($identify) {
			...
			$result = $this->users_model->insert_teacher($account,$password,$phone,$sex,$school,$education);
		}else{
			$result = $this->users_model->insert_student($account,$password,$phone,$sex);
		}
		echo $result;
	}
// 验证注册账号
...
//更改头像
    ...
    if(move_uploaded_file($file['tmp_name'],$upload_path)){
 	...
 	public function check_old(){
 		...
 		$identify=$this->input->post('identify');
 		if (!$identify) {
 			$result=$this->users_model->check_old_student($oldPassword);
 			echo $result ? 1 : 0;
 		} else {
 			$result=$this->users_model->check_old_teacher($oldPassword);
 			echo $result ? 1 : 0;
 		}
 	}
 	//update the password
