  ...
  <div id="reg">
    ...
    <ul>
      <li>
        ...
        <input type="text" placeholder ="请输入用户名" id="account" @blur="t_account()">
      ...
      <li class="sex">
        ...
        <input type="radio" value="男"><span>男</span>
        <input type="radio" value="女"><span>女</span>
      </li>
      <li v-show="identify">
    ...
    <div class="agreement">
      <input type="checkbox" name="agreement"/>我已阅读并同意<router-link to="">《天天有课 用户协议》</router-link>
    </div>
  ...
  methods:{
    ...
    check(){
      ...
      if (this.account && this.phone && this.password && this.password_ && $('input:checked').length == 2) {
        Axios.get('http://localhost:3000/users/reg',{params:obj})
        .then((res)=>{
          if (res.data) {
            alert('注册成功');
