  ...
  <div id="add-class">
    ...
    <div class='add-honor'>
        <div class="honor-list" v-for='(msg,index) in teacherHonor'>
          ...
          <!-- <button class='submit' @click='submit'>添加荣誉</button> -->
          <!-- 弹层图片开始 -->
      <div id="tancen-img" v-if='flag' @click='flag=!flag'>
          <div class="tancen-img">
             <img :src="teacherHonor[index1].honor_img" class="honor-img" >  
             <p class="honor-content"><span>{{teacherHonor[index1].honor_medal}}</span><br><span>{{teacherHonor[index1].honor_time}}</span>
             </p> 
          </div>
      </div>
      <!-- 弹层图片开始结束 -->
    ...
    <div id="honor-img">
        <span class="honor-content">证件类型:&nbsp;</span><input type="text" placeholder="请输入证件类型" v-model='honor.content'><br>
        <span class="honor-content">证件时间:&nbsp;</span><input type="text" placeholder="请输入证件类型" v-model='honor.h_time'>
